
export const services = [
  {
    id: "celulas-madre",
    title: "Terapia con Células Madre",
    shortTitle: "Células Madre",
    description: "Utilización de células madre mesenquimales para regenerar tejidos dañados y tratar enfermedades degenerativas.",
    icon: "fas fa-dna",
    features: [
      "Rejuvenecimiento facial",
      "Regeneración articular",
      "Tratamiento de lesiones"
    ],
    benefits: [
      "Regeneración tisular natural",
      "Tratamiento no invasivo",
      "Resultados duraderos"
    ]
  },
  {
    id: "prp",
    title: "Plasma Rico en Plaquetas",
    shortTitle: "PRP",
    description: "Tratamiento autólogo que utiliza los factores de crecimiento de tu propia sangre para estimular la regeneración celular.",
    icon: "fas fa-syringe",
    features: [
      "Tratamiento capilar",
      "Rejuvenecimiento de piel",
      "Lesiones deportivas"
    ],
    benefits: [
      "100% autólogo y seguro",
      "Sin riesgo de rechazo",
      "Tiempo de recuperación mínimo"
    ]
  },
  {
    id: "medicina-regenerativa",
    title: "Medicina Regenerativa",
    shortTitle: "Medicina Regenerativa",
    description: "Protocolos personalizados que combinan biotecnología avanzada con medicina integrativa para resultados óptimos.",
    icon: "fas fa-dna",
    features: [
      "Tratamientos antienvejecimiento",
      "Detoxificación celular",
      "Optimización hormonal"
    ],
    benefits: [
      "Enfoque integral",
      "Personalización total",
      "Tecnología de punta"
    ]
  },
  {
    id: "terapias-regeneracion",
    title: "Terapias de Regeneración",
    shortTitle: "Terapias Regeneración",
    description: "Tratamientos innovadores que estimulan los procesos naturales de curación y renovación de tu cuerpo.",
    icon: "fas fa-seedling",
    features: [
      "Oxigenoterapia",
      "Terapia de activación celular",
      "Tratamientos complementarios"
    ],
    benefits: [
      "Estimulación natural",
      "Mejora del sistema inmune",
      "Aumento de energía"
    ]
  },
  {
    id: "esteticos",
    title: "Tratamientos Estéticos",
    shortTitle: "Estéticos",
    description: "Procedimientos no invasivos para rejuvenecer tu apariencia utilizando técnicas de bioregeneración.",
    icon: "fa-solid fa-spa",
    features: [
      "Lifting no quirúrgico",
      "Reducción de arrugas",
      "Tratamiento de manchas"
    ],
    benefits: [
      "Sin cirugía",
      "Resultados naturales",
      "Sin tiempo de recuperación"
    ]
  },
  {
    id: "antiaging",
    title: "Medicina Antiaging",
    shortTitle: "Antiaging",
    description: "Enfoque integral para ralentizar el proceso de envejecimiento y optimizar tu calidad de vida.",
    icon: "fas fa-heart-pulse",
    features: [
      "Evaluación gerontológica",
      "Plan de longevidad",
      "Nutraceuticals avanzados"
    ],
    benefits: [
      "Mejora de calidad de vida",
      "Prevención de enfermedades",
      "Longevidad saludable"
    ]
  }
];

export const benefits = [
  {
    id: "natural",
    title: "Tratamientos Naturales",
    description: "Utilizamos los propios recursos de tu cuerpo, minimizando el uso de químicos y medicamentos sintéticos.",
    icon: "fas fa-leaf",
    number: "01"
  },
  {
    id: "minima-invasion",
    title: "Mínima Invasión",
    description: "La mayoría de nuestros procedimientos son no invasivos o mínimamente invasivos, sin tiempo de recuperación prolongado.",
    icon: "fas fa-hand-sparkles",
    number: "02"
  },
  {
    id: "resultados-duraderos",
    title: "Resultados Duraderos",
    description: "Los efectos de la bioregeneración son duraderos y se mantienen con el tiempo, a diferencia de tratamientos temporales.",
    icon: "fas fa-infinity",
    number: "03"
  },
  {
    id: "personalizado",
    title: "Enfoque Personalizado",
    description: "Cada paciente recibe un plan de tratamiento único, diseñado específicamente para sus necesidades y objetivos.",
    icon: "fas fa-user-check",
    number: "04"
  },
  {
    id: "seguridad",
    title: "Seguridad Comprobada",
    description: "Todos nuestros tratamientos cuentan con certificaciones internacionales y protocolos de seguridad rigurosos.",
    icon: "fas fa-shield-heart",
    number: "05"
  },
  {
    id: "equipo-experto",
    title: "Equipo Experto",
    description: "Nuestro equipo médico está compuesto por especialistas reconocidos internacionalmente en medicina regenerativa.",
    icon: "fas fa-users-gear",
    number: "06"
  }
];
